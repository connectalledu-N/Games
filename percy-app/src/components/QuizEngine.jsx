...
export default function QuizEngine({ questions, drachmasPerQ = 5, sectionId, onFinish }) {
  ...
  const next = () => {
    if (isLast) {
      recordComplete(sectionId, score + (showResult && (selected === '__correct__' || selected === q.a) ? 1 : 0), questions.length)
      setDone(true)
    } else {
      setIdx(i => i + 1)
      setSelected(null)
    ...
    return (
      <div className="text-center py-8">
        ...
        <div className="text-gray-300 mb-6">
          You got <span className="text-white font-bold">{finalScore}/{questions.length}</span> correct ({pct}%)
        </div>
        <div className="drachma-coin text-base mx-auto mb-8">
          🪙 +{finalScore * drachmasPerQ} drachmas earned
        </div>
        <button
          onClick={onFinish}
  ...
  return (
    <div>
      ...
      <div className="bg-white/5 rounded-2xl p-6 mb-5">
        ...
        {q.hint && !showResult && (
          <p className="text-sm text-yellow-400/60 mt-2 italic">💡 Hint: {q.hint}</p>
        )}
      </div>
